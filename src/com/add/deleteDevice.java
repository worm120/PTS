	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			...
			for(int i=0;i<ddStrings.length;i++)
			{
				...
				String sql2="select Sample_ID from Sample where Device_ID='"+ddStrings[i]+"'";
				ResultSet rs=ds.select(sql2);
				if(rs!=null)
				{
					while(rs.next())
					{
						String sample_id=rs.getString(1);
						String sql3="delete from SampleAddress where Sample_ID='"+sample_id+"'";
						ds.update(sql3);
					}
				}
