...
// 					}
			...
			var saveHandler = function(window,svg) {
				...
				if(~ua.indexOf('MSIE')) {//from method-draw.js
				//if((~ua.indexOf('Chrome') && $.browser.version >= 533) || ~ua.indexOf('MSIE')) {//该方式不能保存svg,无法弹出svg窗口
					showSourceEditor(0,true);
					return;
				}
				var win = window.open("data:image/svg+xml;base64," + Utils.encode64(svg));
				//var win1 = window.open("e.html");
				/*
				*2014.01.17
				*发送svg至服务器存储
				*anthor:qiye
				*
				*/
				var svgname=prompt("请输入您的名字","Bill Gates");
				if(svgname==null||svgname=="")
				{
					svgname="datetime";
				}
				//$.post("download_svg.jsp", { Action: svg},
				$.post("download_svg.jsp", { Action: svg,name:svgname},
