...
<html>
                ...
                <table class="table-style" id="listtable">
                  <thead>
                    <tr>
                        ...
                        <th class="table-style-th">图像编号</th>
                  ...
                  <tbody>
                  	<%
                  		String pid=request.getParameter("nodeid");//变电所编号
		    			dataselect ds=new dataselect();
		    			String sqls="select * from AlarmLogPicture left join SampleDetail on SampleDetail.Sample_ID=AlarmLogPicture.Sample_ID where AlarmLogPicture.Sample_ID in(select Sample.Sample_ID from Sample where Sample_Type='00' and Sample.Device_ID in(select Devpoint.Device_ID from Devpoint where pid='"+pid+"'))";
		    			...
		    			{	
		    				...
		    				while(rs.next())
		    				{
		    					...
		    					if(rs.getString("AlarmPhoto")!=null&&rs.getString("AlarmPhoto").equals("null")==false)
		    					{
		    						AlarmPhoto=rs.getString("AlarmPhoto").trim();
		    					}
		    					...
		    					+AlarmPhoto+"</td><td class='table-style-td'>"
    ...
    <div id="add" > 
		<form action="#" method="post" id="formsub">
			<h4><span>报警处理</span><span id="close">关闭</span></h4>
    ...
    <script type="text/javascript">
    	function chuli()
    	{
			...
			xmlhttp.open("GET","../servlet/settleAlarm?type=photo&alarmid="+alarmID+"&userid=<%=session.getAttribute("user_id")%>&alarmcontent="+alarmcontent+"&r="+new Date().getTime(),true);
			xmlhttp.send();
    	}
    	function settle(id)
    	{
