...
{
	document.getElementById("formsub").action="addDevice.jsp?sid="+document.getElementById("hid_sid").value+"&DeviceId="+document.getElementById("DeviceId").value+"&DeviceName="+document.getElementById("DeviceName").value;
...
function sure()
{
	...
	{
		alert("设备编号长度不能为空，请重新输入！");
	}
	else if(Device_Name.value.length>=50||Device_Name.value.length<=0)
	{
		alert("设备名称长度不能为空或不能超过50个字符或25个汉字，请重新输入！");
	}
	...
	{
		xmlhttp.open("GET","../servlet/addDevice?Substation_ID="+Substation_ID.options[Substation_ID.selectedIndex].value+"&Device_ID="+Device_ID.value+"&Device_Name="+Device_Name.value+"&Device_Card="+Device_Card.value+"&Device_Place="+Device_Place.value+"&rem="+rem.value+"&type="+type+"&t="+new Date().getTime(),true);
		xmlhttp.send();
	}
